...
func testLTree(ctx *Context, expect string, t *testing.T) {
	...
	val := hex.EncodeToString(ctx.lTree(pk, pubSeed, address(addr)))
	if val != expect {
...
func testGetWotsSeed(ctx *Context, expect string, t *testing.T) {
	...
	val := hex.EncodeToString(ctx.getWotsSeed(skSeed, addr))
	if val != expect {
...
func testGenLeaf(ctx *Context, expect string, t *testing.T) {
	...
	val := hex.EncodeToString(ctx.genLeaf(skSeed, pubSeed, lTreeAddr, otsAddr))
	if val != expect {
...
func testGenSubTree(ctx *Context, expect string, t *testing.T) {
	...
	mt := ctx.genSubTree(skSeed, pubSeed, addr)
